...
\clearpage
\thispagestyle{empty}
\begin{minipage}[t][0.96\textheight][t]{\textwidth}
...
\begin{enumerate}[label={\bfseries\arabic*.}]
  ...
  \item Freshwater crayfish play crucial roles in aquatic ecosystems but are often neglected.
...
\caption{\label{fig-crayfish-distribution}Global native distribution of
...
free-living larval stage. Females carrying eggs and newly hatched
juveniles beneath the abdomen for approximately three weeks before
release, typically at depths of less than 10 m in lakes (Parkyn and
Kusabs 2007). Juveniles mature in two to three years depending on water
temperature (Devcich 1979; Parkyn 2000).
...
Colours represent the trophic state of each lake (Land, Air, Water
Aotearoa (LAWA) 2025).}
...
in R (version 4.6.1; R Core Team (2025)) using the glmmTMB and emmeans
...
\emph{Baseline equivalence} Pre-construction kōura abundance and biomass
did not differ significantly between stone pile and control sites,
confirming baseline equivalence of the paired design (\emph{p} = 0.783
and \emph{p} = 0.571). The two reference sites differed in catfish
abundance (\emph{p} = 0.004) and biomass (\emph{p} = 0.002), reflecting
natural diversity in rocky littoral habitat. Kōura abundance (\emph{p} =
0.968) and biomass (\emph{p} = 0.968) did not differ between reference
sites.

\emph{Kōura response to stone pile construction} Stone pile construction
produced no detectable step-change in kōura abundance, biomass, or
occupancy relative to paired control sites in the 18 months following
deployment (all \emph{p}\textsubscript{adj} = 0.861). A matched-season
comparison, comparing Summer 2025 (before construction) with Summer 2026
(one year after construction), produced the same conclusion (kōura
abundance: \emph{p} = 0.582). Post-construction trajectories did not
diverge significantly between stone pile and control sites (all
\emph{p}\textsubscript{adj} = 0.224). Kōura abundance declined
significantly across post-construction rounds (β = -0.41, \emph{p} =
0.004), driven primarily by the sharp decline at stone pile sites in
Winter 2026 (Figure~\ref{fig-koura-trends}).

\emph{Recovery toward reference conditions} Stone pile sites maintained
a consistently lower kōura abundance than reference sites throughout the
study period, with no significant change in this gap over time (β =
-0.06, \emph{p} = 0.600; Figure~\ref{fig-convergence-plot}). However,
the Winter 2026 monitoring round revealed a widening of this gap, with a
sharp decline of stone pile CPUE (CPUE ≈ 0.7) while reference sites
maintained substantially higher abundance (CPUE ≈ 1.7;
Figure~\ref{fig-koura-trends}), a contrast not captured by the linear
trajectory model. Winter significantly suppressed kōura abundance at
reference sites (β = -0.65, \emph{p} = 0.001), yet reference sites
remained more resilient than stone pile sites. Water temperature at
Winter 2026 monitoring was approximately 1.5°C lower than at Winter 2025
across all site types, and was consistent across site types and
therefore cannot explain the collapse at stone pile sites. Visual
inspection of stone pile surfaces at Winter 2026 monitoring revealt
substantial greater algal and biomass accumulation compared to earlier
rounds, suggesting progressive structural fouling at the stone pile
surface over the study period.
...
\emph{Short-term construction effect} Stone pile construction to
increase refuge habitat for kōura in the littoral zone of Lake Rotoiti
was seasonally monitored for 18 months after deployment of the stone
piles. This monitoring window is likely insufficient for kōura habitat
establishment. Other studies showed it can take 13 years for crayfish
densities to reach similar levels on modified sites as control sites
(Johnsen and Taugbøl 2008). Also in Lake Rotorua after reconstruction of
Rotorua Lake Front in July 2021 it took 1.5 years for kōura to
recolonise the lake front. Most freshwater invertabre colonosation
studies show effects at 2-3 years of monitoring (\textbf{BRON?}).
...
stratification compresses the zone of livable habitat for kōura into the
shallow littoral margins of the lake, this is the same zone where
...
as bottom-dwelling animals, the constraint is more severe as livable
...
Nevertheless the lessons learned were of great value for the further
...
substrte has been lost or never existed are therefor logical candidates
for stone pile restoration. Chapter 4 confirmed that stability and
structural complexity are the decisive factors in kōura shelter
...
case study. Whether habitat restoration achieves those benefits depends
on whether habitat is actually the limiting factor. In a Norwegian lake
where muddy substrates limited noble crayfish (\emph{Astacus astacus})
distribution, adding rocky substrate to previously unproductive areas
resulted in colonisation and improved harvest (Johnsen and Taugbøl
2008). In that system, habitat was the constraint, and removing it
produced a measurable response. Similarly, structure additions to
streams have been shown to improve success of crayfish restocking
efforst (Huolila et al. 1997). In both cases, the stressor being
addressed was the primary one operating on the population. The situation
in the Rotorua Te Arawa Lakes is more complex. Lack of suitable habitat
is one of the compounding stressors, and addressing it alone will not
produce the same clear response. If dissolved oxygen drops below kōura
tolerance, structure is irrelevant as demonstrated by the anoxic event
in Lake Ōkaro. If catfish pressure remains high, stone piles improve
survival odds but cannot prevent the ongoing suppression from predation
and fear effects. Restoration therefore sets a ceiling defined by the
other stressors: Chapter 3 showed that the lakes with the best habitat
and the best water quality, most complex habitat, and no catfish support
the highest kōura abundance. The question is not whether to restore
habitat, but at what point in a lake's recovery trajectory habitat
becomes the limiting factor, and before that point, improving water
quality and reducing predator pressure will produce greater return. The
sequence of kōura population recovery would start with stabilising water
quality to a point where it falls within kōura acceptable limits, then
focus would need to be on predator management, lowering the predation
pressure and then habitat recovery and if needed reintroduction.

\section{Future direction}\label{future-direction}

What have we learned for the future\ldots{}
