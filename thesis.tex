...
(Figure~\ref{fig-forest-plot}), indicating that stone piles did not
attract catfish.
...
(columns) and research question (rows). Filled points = significant
(\emph{p} \textless{} 0.05); open points = non-significant.
\textbf{Before: stone pile vs control} --- baseline group difference
(stone pile minus control) before construction. \textbf{Before:
reference 1 vs reference 2} --- baseline comparability between the two
reference groups. \textbf{Before-After: BACI} --- the BACI interaction:
did stone pile sites change more than control sites from before to after
construction? \textbf{After: trajectory} --- per-round slope difference
between stone pile and control sites in the post-construction period.
\textbf{Convergence toward reference} --- per-round slope difference
between stone pile and reference sites post-construction.
\textbf{Reference stability} --- per-round slope at reference sites,
testing whether reference conditions were stable. Each row uses its own
x-axis scale; effect magnitudes are not comparable across rows as each
tests a different contrast (step-change, per-round slope, or simple
group difference), and occupancy is on the log-odds scale while
abundance and biomass use the Tweedie log link.}
