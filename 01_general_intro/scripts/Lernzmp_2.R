...
for (i in seq_along(aeme_list)) {
  ...
  hyps$total_area <- lake_metadata$Area
...
# Optionally, summarize or filter the data (e.g., mean values by variable and lake)
...
ggplot(water_quality_data |> filter(lake_name == "Rotoiti (Bay of Plenty)", var_aeme == "CHM_oxy"), 
       aes(y = depth_mid, x = value, col=Date)) +
  geom_point()+
  scale_y_reverse()
